...
def zbx_host_processing():
    # Obtener la ubicación del script actual
    script_directory = os.path.dirname(os.path.abspath(__file__))

    # Retroceder un nivel para llegar al directorio principal del proyecto
    project_directory = os.path.dirname(script_directory)

    # Ruta relativa al directorio de archivos JSON
    json_directory = os.path.join(project_directory, 'json_files')
    ...
    output_file_path_reachable = os.path.join(json_directory, 'hosts_reachable.json')
    output_file_path_rejected = os.path.join(json_directory, 'hosts_rejected.json')
    ...
    print(
        f"Se han almacenado {len(reachable_hosts)} hosts con conectividad en el archivo 'hosts_reachable.json'.")
    print(f"Se han almacenado {len(rejected_hosts)} hosts rechazados en el archivo 'hosts_rejected.json'.")
